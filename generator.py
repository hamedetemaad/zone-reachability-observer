...
def generate_ebpf_with_interface(interface_name):
    input_ebpf_template = 'xdp.bpf.c.tpl'

    output_ebpf = './xdp/xdp_' + interface_name + '.bpf.c'

    try:
        with open(input_ebpf_template, 'r') as template_file:
            ebpf_content = template_file.read()

        modified_content = ebpf_content.replace('INTERFACE', interface_name)

        with open(output_ebpf, 'w') as output_ebpf:
            output_ebpf.write(modified_content)
...
if __name__ == '__main__':
    inventory = ""
    job = "- job_name: \"zone_reachability_monitoring\"" + "\n  " + "static_configs:" + "\n"
    zmon_config_path = './zmon.cfg'
    i=1
    with open(zmon_config_path, 'r') as file:
        data = json.load(file)
        
    ebpf_exporter_port = data["ebpf_exporter_port"]
    for zone_name, zone_info in data["zones"].items():
        inventory += '[' + zone_name + "]\n"
        job += '    - targets: ['
        for server in zone_info['servers']:
          inventory += 'server'+ str(i) + ' ansible_host=' + server['ip'] + ' interface=' + server['interface']  + "\n"
          i+=1
          generate_ebpf_with_interface(server["interface"])
          job += '"' + server["ip"] + ':' + ebpf_exporter_port  + '",'
        job = job[:job.rfind(",")]
        job += ']' + "\n"
        job += '      labels:' + "\n" + '        dest: ' + "'" + zone_name + "'\n"
    ...
    for zone_name, zone_info in data["zones"].items():
        job += "    - source_labels: ['from']\n"
        job += '      regex: "' + cidr_to_regex(zone_info['cidr'])+ "\"\n"
        job += "      action: replace\n"
        job += '      target_label: "source"' + "\n"
        job += '      replacement: "' + zone_name + "\"\n"
        inventory += zone_name + "\n"
    generate_inventory_file()
    print("#########################################")
    print("#Prometheus Job Config")
    print("#########################################\n")
    job = job.replace("\.", ".")
    print(job)
